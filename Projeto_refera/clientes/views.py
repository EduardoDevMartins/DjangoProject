...
def index(request):
    Clientes = Cliente.objects.order_by('Data_atrelamento')
...
def busca(request):
    ...
    Clientes = Cliente.objects.order_by('Data_atrelamento').filter(
        Q(Chamado__icontains=termo) | Q(Data_atrelamento__icontains=termo),
        Ativo=True
